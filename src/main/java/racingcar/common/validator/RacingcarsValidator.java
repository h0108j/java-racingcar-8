...
import static racingcar.common.Constants.MINIMUM_RACINGCAR_QUANTITY;
import static racingcar.common.Exceptions.*;
...
public class RacingcarsValidator implements Validator<List<Racingcar>>{
    ...
    private void validateRacingcarQuantity(List<Racingcar> racingcarList) {
        if (racingcarList.size() > MAXIMUM_RACINGCAR_QUANTITY) {
            throw new IllegalArgumentException(OVER_RACINGCAR_MAXIMUM_EXCEPTION);
        } else if (racingcarList.size() < MINIMUM_RACINGCAR_QUANTITY) {
            throw new IllegalArgumentException(UNDER_RACINGCAR_MINIMUM_EXCEPTION);
        }
    }
